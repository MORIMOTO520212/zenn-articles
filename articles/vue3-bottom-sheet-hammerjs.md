...
```mermaid
flowchart TB
    A["open（画面の 1% の位置）<br>コンテンツ全表示"]
    B["half（画面の 45% の位置）<br>▬▬▬ ドラッグバー"]
    C["close（画面の 70% の位置）<br>少しだけ見える"]
...
const calcY = computed(() => {
  ...
  switch (state.value) {
    case "open":
      return rect.value.height * props.openY;
    case "half":
      return rect.value.height * props.halfY;
    case "close":
      return rect.value.height * props.closeY;
  }
});
```
